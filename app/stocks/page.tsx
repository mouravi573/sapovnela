...
export default function StocksDashboard() {
  ...
  const loadStocks = useCallback(async () => {
    try {
      ...
      setStats({ total: data.total_stock_markets ?? 0, priced: data.tickers_priced ?? 0 })
      setStatus('Live')
    } catch {
      setStatus('Error')
    }
    ...
    <div style={{ minHeight: '100vh', background: '#080b0e', color: '#dde3ed' }}>
      ...
      <div style={{ maxWidth: 1040, margin: '0 auto', padding: '0 24px 80px' }}>
        <nav style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '22px 0 20px', borderBottom: '1px solid #1a2230', marginBottom: 40 }}>
          <div className="st-mono" style={{ fontSize: 13, fontWeight: 600, color: '#00c9a7', letterSpacing: '0.08em', textTransform: 'uppercase' }}>
        ...
        {withEdge.length > 0 && (
          <section style={{ marginBottom: 32 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {withEdge.map(s => (
                <div key={s.condition_id} style={{ background: '#0e1318', border: '1px solid rgba(245,166,35,0.3)', padding: 16 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16 }}>
                    <div>
                      <div style={{ fontSize: 13, fontWeight: 500, marginBottom: 6 }}>{s.question}</div>
                      <div className="st-mono" style={{ fontSize: 11, color: '#8892a4' }}>
                        {s.ticker} real: <span style={{ color: '#00c9a7' }}>${s.real_price?.toFixed(2)}</span>
                        {' · '}strike: <span style={{ color: '#f5a623' }}>${s.strike_price?.toFixed(2)}</span>
                        {' · '}poly: <span style={{ color: '#8892a4' }}>{(s.yes_price * 100).toFixed(1)}¢</span>
                      </div>
                    </div>
                    <div style={{ textAlign: 'right' }}>
                      <div className="st-mono" style={{ fontSize: 16, fontWeight: 600, color: '#3dd68c' }}>+{((s.edge ?? 0) * 100).toFixed(1)}¢</div>
                      <button
                        onClick={() => prefillTrade(s.question, 'YES', s.yes_price * 100)}
                        className="st-mono"
                        style={{ fontSize: 9, letterSpacing: '0.06em', textTransform: 'uppercase', padding: '5px 12px', border: '1px solid #00c9a7', background: 'rgba(0,201,167,0.08)', color: '#00c9a7', cursor: 'pointer', marginTop: 6 }}
                      >
                        Log trade →
                      </button>
                    </div>
        ...
        <section style={{ marginBottom: 32 }}>
          ...
          {withoutEdge.length === 0 && withEdge.length === 0 ? (
            ...
            <div style={{ background: '#0e1318', border: '1px solid #1a2230', overflow: 'hidden' }}>
              <div className="st-mono" style={{ display: 'grid', gridTemplateColumns: '1fr 80px 90px 90px 100px', gap: 12, padding: '10px 16px', fontSize: 9, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#4a5568', borderBottom: '1px solid #1a2230' }}>
                <div>Question</div><div>Ticker</div><div>Real price</div><div>Poly</div><div>Volume</div>
              </div>
              {withoutEdge.map(s => (
                <div
                  key={s.condition_id}
                  className="st-row"
                  onClick={() => prefillTrade(s.question, 'YES', s.yes_price * 100)}
                  style={{ display: 'grid', gridTemplateColumns: '1fr 80px 90px 90px 100px', gap: 12, padding: '12px 16px', borderBottom: '1px solid #1a2230', cursor: 'pointer', alignItems: 'center' }}
                >
                  <div style={{ fontSize: 12 }}>{s.question}</div>
                  <div className="st-mono" style={{ fontSize: 11, color: '#8892a4' }}>{s.ticker ?? '—'}</div>
                  <div className="st-mono" style={{ fontSize: 11, color: '#00c9a7' }}>{s.real_price ? `$${s.real_price.toFixed(2)}` : '—'}</div>
                  <div className="st-mono" style={{ fontSize: 11, color: '#8892a4' }}>{(s.yes_price * 100).toFixed(1)}¢</div>
                  <div className="st-mono" style={{ fontSize: 11, color: '#4a5568' }}>${(s.volume_24h / 1000).toFixed(0)}K/24h</div>
                </div>
              ))}
            </div>
          )}
