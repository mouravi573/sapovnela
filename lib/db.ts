...
import type {
  PolyMarket, Signal, Order, Position,
  SystemStats, CalibrationBucket
} from '@/types'
...
export async function getMarketById(conditionId: string): Promise<PolyMarket | null> {
...
export async function getSystemStats(): Promise<SystemStats> {
  const [marketsRes, signalsRes, positionsRes, closedRes] = await Promise.all([
    ...
    supabase.from('signals').select('*', { count: 'exact', head: true }).eq('acted_on', false).gt('expires_at', new Date(Date.now()).toISOString()),
    supabase.from('positions').select('*').eq('status', 'open'),
    supabase.from('positions').select('realized_pnl, resolution').in('status', ['closed', 'resolved']),
  ])

  const openPositions = (positionsRes.data ?? []) as Position[]
  const closedPositions = (closedRes.data ?? []) as Array<{ realized_pnl: number; resolution: string }>
...
export async function getCalibration(): Promise<CalibrationBucket[]> {
  ...
  for (const p of data as Array<{ resolution: string; avg_entry_price: number; side: string }>) {
    const pred = Math.round(p.avg_entry_price * 10) * 10 // nearest 10%
    const key = `${pred - 10}-${pred}%`
    if (!buckets[key]) buckets[key] = { wins: 0, total: 0, mid: (pred - 5) / 100 }
    buckets[key].total++
